...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.8.25") {
        throw new Error(`Cannot start service: Host version "${"0.8.25"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
...
function referenceCountedService(startService2) {
  let entries = new Map();
  return (options) => __async(this, null, function* () {
    ...
    if (entry === void 0) {
      entry = {promise: startService2(JSON.parse(key)), refCount: 0};
...
var startService = referenceCountedService((options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.8.25"}`), {
    cwd: process.cwd(),
    windowsHide: true,
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.8.25"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
